...
from vl_mamba.datamodels.datamodels import DatasetFeatures, DatasetSplits, DocVQAMetadata, Task
...
class DocVQALoader(BaseLoader):
    ...
    image_urls = (
        "https://huggingface.co/datasets/gpantaz/docvqa/resolve/main/spdocvqa_images.tar.gz"
    )

    question_urls = "https://huggingface.co/datasets/gpantaz/docvqa/resolve/main/spdocvqa_qas.zip"

    question_files = {
        DatasetSplits.TRAIN: "train_v1.0_withQT.json",
        DatasetSplits.VALIDATION: "val_v1.0_withQT.json",
        DatasetSplits.TEST: "test_v1.0.json",
    }
    ...
    def _build_rows_iterator(self, chunk_size: int) -> Iterator[list[Any]]:  # noqa: WPS210
        ...
        if self.split in {DatasetSplits.TRAIN, DatasetSplits.VALIDATION}:
            grouped_annotations = groupby(questions_metadata["data"], key=lambda x: x["image"])
        # Dont pack the questions in the the test split
        else:
            grouped_annotations = [
                (metadata["image"], [metadata]) for metadata in questions_metadata["data"]
            ]
        ...
        for image_id, questions_iterator in grouped_annotations:
            ...
            for raw_question in questions_iterator:
                question_metadata = DocVQAMetadata.model_validate(raw_question)
                answer = None
            ...
            buffer.append(
                {
                    ...
                    "metadata": {
                        "image_path": str(Path(self.image_folder_path, Path(image_id).name)),
                        "question_types": [metadata.question_types for metadata in all_metadata],
                        "doc_id": [metadata.doc_id for metadata in all_metadata],
                        "ucsf_document_id": [
                            metadata.ucsf_document_id for metadata in all_metadata
                        ],
                        "ucsf_document_page_no": [
                            metadata.ucsf_document_page_no for metadata in all_metadata
                        ],
