...
if is_accelerate_available():
    from accelerate import __version__ as accelerate_version, skip_first_batches
...
class CustomTrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        all_inputs = None
        all_correct = 0
        all_correct_per_position = torch.zeros(self.seq_len)
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels, correct, correct_per_position = self.prediction_step(
            ...
            if correct is not None:
                ...
                all_correct += self.gather_function(correct)
            
            if all_correct_per_position is not None:
                try:
                    all_correct_per_position += self.gather_function(correct_per_position)
                # For some reason the last batch is not processed correctly
                except:
                    all_correct_per_position += correct_per_position
    ...
    def _inner_training_loop(
        ...
        if trial is not None:
            assignments = (
                trial.assignments if self.hp_search_backend == HPSearchBackend.SIGOPT else trial
            )
